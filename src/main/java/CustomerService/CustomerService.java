...
//
    ...
    //GetUsername
    public UserStudent getUsername( int id){
        UserStudent student=null;
        String query="select username from cv where id=?";
        PreparedStatement ps = new DBConnection().getStatement(query);
        try{
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
    ...
    //EditUsername
    public void Editname( UserStudent student, int id){
        String query = "UPDATE cv SET username = ? WHERE  id= ?";
        PreparedStatement ps = new DBConnection().getStatement(query);
        try{
